...
int MTCNN::LoadModel(AAssetManager* assetManager) {
    //binited 0表示false，非0值表示true
    LOG_ERR("MTCNN, mtcnn = %s = %b", "model loading", binited);
    if (binited)
        release();

    int ret = Pnet.load_param(assetManager, "mtcnn/det1.param");
    ret = Pnet.load_model(assetManager, "mtcnn/det1.bin");
    ret = Rnet.load_param(assetManager, "mtcnn/det2.param");
    ret = Rnet.load_model(assetManager, "mtcnn/det2.bin");
    ret = Onet.load_param(assetManager, "mtcnn/det3.param");
    ret = Onet.load_model(assetManager, "mtcnn/det3.bin");

    if (ret == 0) {
        LOG_INFO("MTCNN, mtcnn = %s", "model load success");
    } else {
        LOG_ERR("MTCNN, mtcnn = %s", "model load failed");
    }
    binited = true;
    return ret;
}
...
void MTCNN::detectMaxFace(ncnn::Mat &img_, std::vector<Bbox> &finalBbox) {
    ...
    for (size_t i = 0; i < scales_.size(); i++) {
        ...
        if (get_current_time() - start_time > 1000000) { // 1秒 = 1,000,000 微秒
            // 如果超时，则中断后续处理
            LOG_ERR("Detection timeout after 1 second");
...
//    std::string filePath = "/storage/emulated/0/show.jpg";
//    cv::imwrite(filePath, show);
    ...
    for (int i = 0; i < num_box; i++) {
        ...
        for (int j = 0; j < 5; j = j + 1) {
            cv::circle(img, cv::Point(box[i].ppoint[j], box[i].ppoint[j + 5]), 2, {0, 0, 255});
        }
    }
...
//    ncnn_img = img_;
//    // 设置输入
//    ncnn::Extractor ex = AgeNet.create_extractor();
...
//    LOG_ERR("ncnn detectAge cost = %lld ms ", duration_ms);
//    return age;
//}